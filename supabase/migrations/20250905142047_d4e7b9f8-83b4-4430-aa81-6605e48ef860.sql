-- Update existing inventory table to match requested schema
ALTER TABLE public.inventory 
DROP COLUMN IF EXISTS id,
DROP COLUMN IF EXISTS description,
DROP COLUMN IF EXISTS quantity;

-- Add missing columns to match the requested schema
ALTER TABLE public.inventory 
...
-- Ensure sku is primary key
ALTER TABLE public.inventory DROP CONSTRAINT IF EXISTS inventory_pkey;
ALTER TABLE public.inventory ADD PRIMARY KEY (sku);
...
DROP POLICY IF EXISTS "Inventory delete by admins" ON public.inventory;
DROP POLICY IF EXISTS "Inventory is readable by authenticated" ON public.inventory;
DROP POLICY IF EXISTS "Inventory update by editors and admins" ON public.inventory;

-- Create new RLS policy for authenticated users to SELECT
CREATE POLICY "Authenticated users can view inventory" 
ON public.inventory 
FOR SELECT 
USING (auth.role() = 'authenticated');

-- No INSERT/UPDATE/DELETE policies = blocked for regular users (service role only)

-- Clear existing data and seed with TSV data
TRUNCATE public.inventory;

-- Insert TSV data with UPSERT
